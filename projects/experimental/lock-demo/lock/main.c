...
void rx_task ()
{
  ...
  while (1) {
    ...
    for (i = 0; i < len; i++)
	{
	if((rx_buf[i]-1)!=tx_buf[i] ) decoded=0;
	}
    if(decoded==1) unlock=30;
...
val=nrk_gpio_direction(NRK_PORTF_0, NRK_PIN_OUTPUT);
  ...
  while (1) {
    ...
    // Build a TX packet
    if(cnt%10==0) sprintf (tx_buf, "This is a test %d", cnt);
    ...
    if(unlock==0 ) { printf( "Door locked\r\n" );  nrk_gpio_set(NRK_PORTF_0); nrk_led_set(RED_LED); nrk_led_clr(GREEN_LED); }
	else 
	{
		printf( "Door unlocked\r\n" );
		nrk_gpio_clr(NRK_PORTF_0);
