...
uint8_t publish_node_mio(mio_conn_t *conn, node_t *n);
...
static void XMLCALL XMLstart_read_conf_optional(void *data,
	...
	if (strcmp(element_name, "regMap") == 0) {
		...
		// Set the publish flag on the transducer to true so that it will be published via MIO
		t->publish = 1;
...
node_t *process_pkt(PKT_T *pkt) {
	...
	// Update timestamp
	timestamp = mio_timestamp_create();
	if (n->timestamp != NULL )
		free(n->timestamp);
...
uint8_t publish_node_mio(mio_conn_t *conn, node_t *n) {
	char value[MAX_STRING_LONG];
	char id[MAX_STRING_LONG];
	mio_stanza_t *item;
	transducer_t *t;
	mio_transducer_data_t *data, *td;
...
// Only publish transducervalue if it's publish flag is set
		if (t->publish) {
			...
			snprintf(value, MAX_STRING_LONG, "%"FMT_SIZE_T"", t->raw_val);
			td = mio_transducer_data_new();
			td->type = (mio_transducer_data_type_t) MIO_TRANSDUCER_DATA;
			td->value = value;
			td->name = id;
			mio_transducer_data_add(data, td);
		}
	}

	item = mio_transducer_data_to_item(conn,t);
// Publish and free stanza
	mio_item_publish_nonblocking(conn, item, n->node);
	for (td = data; td != NULL; td = td->next)
		mio_transducer_data_free(td);
...
void write_vals_csv(node_t *n) {
	...
	for (t = n->transducers; t != NULL ; t = t->hh.next)
		fprintf(csv_fp, "%s,%"FMT_SIZE_T",%f", t->name, t->raw_val, t->val);
	fprintf(csv_fp, "\n");
	fflush(csv_fp);
...
void *actuation_watcher(void *ud) {
	...
	mio_transducer_data_t *td = NULL, *td_pub;
	mio_response_t *response, *pub_response;
	node_t *n = NULL;
	transducer_t *t = NULL;
	char *event;
	int v, underscore_pos = -1;
	int err;

	while (1) {
		if (global_conn->xmpp_conn->state == XMPP_STATE_CONNECTED) {
			response = mio_response_new();
			err = mio_pubsub_data_receive(global_conn, response);

			if (err == MIO_ERROR_NO_RESPONSE) {
				mio_response_free(response);
				continue;
			}
			...
			for (td = data->transducers; td != NULL ; td = td->next) {
				if (td->type == MIO_TRANSDUCER_SET_DATA) {
					// Find underscore from _actuation in event node name and copy only UUID into event string
					underscore_pos = strcspn(data->event, "_");
					if (underscore_pos > 0) {
						event = malloc(
								sizeof(char) * (strlen(data->event) + 1));
						strncpy(event, data->event, underscore_pos);
						underscore_pos = -1;
					} else
						event = strdup(data->event);
					// Search for MAC of node corresponding to its id if we haven't found it already
					HASH_FIND(hh_node, nodes, event,
							sizeof(char) * strlen(event), n);
					if (n != NULL && td->name != NULL ) {
						if (strcmp(td->name, "Socket State") == 0) {
							fprintf(stderr,
									"Got socket state transducerSetData packet\n");
							...
							if (t != NULL ) {
								// Create new publication which sets actuator to transitioning state
								td_pub = mio_transducer_data_new();
								td_pub->name = strdup(td->name);
								td_pub->value = strdup("-1");
								td_pub->type = MIO_TRANSDUCER_DATA;
								td_pub->timestamp = mio_timestamp_create();
								pub_response = mio_response_new();

								if (strcmp(td->value, "0") == 0) {
									outlet_actuate(n->mac, 0, 0);
//									t->raw_val = 0;
//									t->val = 0;
//									mio_transducer_value_merge_publish(
//											global_conn, tv_pub, event,
//											pub_response);
								} else if (strcmp(td->value, "1") == 0) {
									// Actuate outlet and update
...
//											pub_response);
								...
								free(event);
								mio_transducer_data_free(td_pub);
								mio_response_free(pub_response);
							}
						} else if (strcmp(td->name, "GPIO Mask") == 0) {
							v = atoi(td->value);
							gpio_actuate(n->mac, 0, 0, v);
						} else if (strcmp(td->name, "GPIO Set0") == 0) {
							v = atoi(td->value);
							gpio_actuate(n->mac, 1, 0, v);
						} else if (strcmp(td->name, "GPIO Set1") == 0) {
							v = atoi(td->value);
							gpio_actuate(n->mac, 1, 1, v);
						}
						// If we receive a ping command, send it out
						else if (strcmp(td->name, "Ping") == 0)
							send_ping(n->mac, atoi(td->value));
			...
			if (v > 0) {
				...
				if (n != NULL ) {
					if (csv != NULL )
						write_vals_csv(n);
					//if (n->publish)
					publish_node_mio(global_conn, n);
				}
			}
		}
#endif
...
void print_usage(char *prog_name) {
#if defined(USE_MIO)
	fprintf(stdout, "\"%s\" SLIPstream MIO-CSV client\n", prog_name);
	fprintf(stdout,
			"Usage: %s <-s SLIPstream_server_address> <-port SLIPstream_server_port> [-mio mio_configuration_file] [-mac_node mac_node_configuration_file] [-u XMPP username -p XMPP password] [-csv CSV output file] [-verbose]\n",
			prog_name);
#else
	...
	fprintf(stdout, "\t-mio MIO configuration file\n");
#ifdef USE_MIO
	fprintf(stdout,
...
// Set our presence status to unavailable and disconnect xmpp connection
	if (global_conn != NULL ) {
		mio_disconnect(global_conn);
		mio_conn_free(global_conn);
	}
	exit(1);
}
...
int main(int argc, char *argv[]) {
	...
	underscore_pos = strcspn("12345_actuation", "_");
						if (underscore_pos > 0) {
							event = malloc(
									sizeof(char) * (strlen("12345_actuation") + 1));
							strncpy(event, "12345_actuation", underscore_pos);
							underscore_pos = -1;
						} else
							event = strdup("12345_actuation");
...
// Add SIGINT handler
	...
	while (current_arg_num < argc) {
		...
		} else if (strcmp(current_arg_name, "-mio") == 0) {
			opt = current_arg_val;
	...
	if (username == NULL && password != NULL ) {
		...
		fprintf(stderr,
				"Either MIO and/or CSV output functionality is required\n");
		print_usage(argv[0]);
		return GW_ERROR;
	}
#else
