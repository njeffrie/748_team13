...
struct hostent *hp;
int listenfd,connfd, length, fromlen,replylen, n,reply_sock;
...
//  fcntl (listenfd, F_SETFL, O_NONBLOCK);
  ...
  if (bind (listenfd, (struct sockaddr *) &server, length) < 0)
    perror ("binding");
...
void server_non_blocking_rx (int fd)
{
  ...
  n = recvfrom (connfd, buf, 1024, 0, (struct sockaddr *) &from, &fromlen);
...
json_object_set_new(root, "ttl", json_integer(p->ttl));
json_object_set_new(root, "src_address", json_integer(p->src_mac));
json_object_set_new(root, "dst_address", json_integer(p->dst_mac));
json_object_set_new(root, "seq_num", json_integer(p->seq_num));
json_object_set_new(root, "proto_version", json_integer(p->proto_version));

printf( "App Payload len: %d\n",p->payload_len );
for(i=0; i<p->payload_len; i++ ) printf( "%d ", p->payload[i] );
printf( "\n" );
...
void server_tx (uint8_t * buf, uint8_t size)
{
  ...
  v=sm_buf_to_pkt(buf,&my_pkt );


  if(v>0)
  {
        json_str=generate_json(&my_pkt);
	printf( "%s\n",json_str );
...
//	n = sendto (connfd, json_str, strlen(json_str)+1, 0, (struct sockaddr *) &client, replylen);
	n = sendto (connfd, json_str, strlen(json_str), 0, (struct sockaddr *) &cliaddr, sizeof(cliaddr));
...
void print_usage ()
{
  printf ("Usage: SLIPstreamJ com-port port <-d or -s> <-a client-address>\n");
  printf ("  Ex: SLIPstreamJ /dev/ttyUSB0 4000\n");
