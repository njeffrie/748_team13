// Add this option for non-multithreaded operation
// It runs the UART in non-blocking mode which fixes some issues
// opening the FTDI driver in OS X.
#define NONBLOCKING 
...
//void server_rx_handler(int fd);
void *server_rx_handler(void *arg);
void server_tx_handler(int fd);

struct hostent *hp;
int sock, length, replylen, n,reply_sock;
socklen_t fromlen;
...
int main (int Parm_Count, char *Parms[])
{
  ...
  if (Parm_Count < 3 || Parm_Count > 6)
    print_usage ();
  strcpy ((char *)buf, Parms[1]);
  i = sscanf ((char *)buf, "%s", devicename);
  if (i != 1)
    print_usage ();
...
#else
  // open the device with blocking default 
  // This fails in OS X... 
  ...
  strcpy ((char *)buf, Parms[2]);
  i = sscanf ((char *)buf, "%d", &port_num);
  if (i != 1)
    print_usage ();
  debug = 0;
  tmp_pcount=3;
  ...
  {
    strcpy ((char *)buf, Parms[tmp_pcount]);
    i = sscanf ((char *)buf, "%s", debug_flag);
    if (i != 1)
      print_usage ();
...
#else
pthread_create(&rx_thread, NULL, server_rx_handler, &fd);

server_tx_handler(fd);
